 ...
 * although the API currently reads the single active presentation.
 */
async function readActiveFiles() {
  const status = document.getElementById("status");
  if (status) {
    status.textContent = "Reading active file(s)...";
    // Allow UI to render the initial message
  ...
  try {
    ...
    if (status) status.textContent = `File size: ${fileSize} bytes. Reading ${sliceCount} slices from active presentation(s)...`;
