 ...
 * although the Office JS API (getFileAsync) is technically limited
 * to the single active document hosting the add-in.
 */
async function readActiveFiles() {
  ...
  try {
    ...
    // Small delay to ensure the UI renders the file size info
    await new Promise(resolve => setTimeout(resolve, 10));
    ...
    for (let i = 0; i < sliceCount; i++) {
      ...
      // Small delay to ensure the UI renders the progress
      await new Promise(resolve => setTimeout(resolve, 10));
    }
    ...
    // Small delay to ensure the UI renders the success message
    await new Promise(resolve => setTimeout(resolve, 10));
