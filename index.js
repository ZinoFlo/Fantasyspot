 ...
 * Note: Plural terminology (file(s), presentation(s)) is used for design consistency,
 * although the current implementation focuses on the single active presentation.
 */
async function readActiveFiles() {
  const status = document.getElementById("status");
  if (status) status.textContent = "Reading active file(s)...";

  // Use a short delay to ensure the UI renders the status message before proceeding.
  ...
  try {
    ...
    for (let i = 0; i < sliceCount; i++) {
      ...
      // Short delay to allow the UI to render the progress before the next slice is processed.
