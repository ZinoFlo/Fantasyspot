...
async function readActiveFiles() {
  ...
  if (status) status.textContent = "Reading active presentation(s)...";
  ...
  try {
    ...
    if (status) {
      status.textContent = `Successfully read active presentation: ${fileSize} bytes.`;
    }
    console.log(`Read ${fileSize} bytes from the active presentation(s).`);
  } catch (error) {
    const errorMsg = `Error reading presentation(s): ${error.message || error}`;
    console.error(errorMsg);
    if (status) status.textContent = errorMsg;
  } finally {
