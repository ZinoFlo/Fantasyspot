 ...
 * Note: While the Office JS API (getFileAsync) is technically limited to the single
 * active document hosting the add-in, pluralized terminology is used here for
 * design consistency across the Eco-growth Discovery platform.
 */
async function readActiveFiles() {
  ...
  try {
    ...
    if (status) status.textContent = `Total size: ${fileSize} bytes. Reading ${sliceCount} slice(s)...`;
