 ...
 *
 * Note: While the UI and functions use pluralized terminology (e.g., "active file(s)")
 * for design consistency, Office.context.document.getFileAsync is currently limited
 * to the single active presentation that is hosting the add-in.
 */
async function readActiveFiles() {
  const status = document.getElementById("status");
  if (status) status.textContent = "Reading active file(s)...";

  // Small delay to ensure the UI renders the message
  ...
  try {
    ...
    for (let i = 0; i < sliceCount; i++) {
      ...
      // Small delay to allow for UI progress updates
      await new Promise(resolve => setTimeout(resolve, 10));
    }
