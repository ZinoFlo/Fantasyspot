 ...
 * Note: Plural terminology is used for UI consistency, though getFileAsync
 * currently targets the single active presentation.
 */
async function readActiveFiles() {
  const status = document.getElementById("status");
  if (status) status.textContent = "Reading active file(s)...";

  // Small delay to ensure UI renders the initial status
