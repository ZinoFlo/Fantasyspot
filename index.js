...
/**
 * Reads the active presentation(s) or file(s) as a compressed byte stream.
 * Note: Pluralized terminology ('active file(s)', 'presentation(s)') is used for design consistency,
 * although the Office JS API (getFileAsync) is technically limited to the single active document hosting the add-in.
 */
async function readActiveFiles() {
  ...
  try {
    ...
    if (status) status.textContent = `Active file size: ${fileSize} bytes. Reading ${sliceCount} slices...`;
