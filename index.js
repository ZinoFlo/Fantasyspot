 ...
 * the implementation is limited by the Office JS API to the single active presentation.
 */
async function readActiveFiles() {
  ...
  try {
    ...
    if (status) {
      status.textContent = `Successfully read active file(s): ${fileSize} bytes (${sliceCount} slices).`;
    }
    console.log(`Read ${fileSize} bytes (${sliceCount} slices) from the active presentation(s).`);
  } catch (error) {
