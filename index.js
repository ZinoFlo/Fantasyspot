 ...
 * Note: Pluralized terminology is used for design consistency, although
 * the Office JS API is technically limited to the single active document hosting the add-in.
 */
async function readActiveFiles() {
  ...
  try {
    ...
    if (status) status.textContent = `File size: ${fileSize} bytes. Reading ${sliceCount} slices from the presentation(s)...`;
