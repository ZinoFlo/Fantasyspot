 ...
 * Note: Pluralized terminology is used for design consistency, despite the
 * technical limitation of getFileAsync to the single active presentation.
 */
async function readActiveFiles() {
  ...
  try {
    ...
    if (status) status.textContent = `File size: ${fileSize} bytes. Reading ${sliceCount} slices from the active presentation(s)...`;
